...
def main():
    ...
    '''''''''
    #企業１社の案件情報をざっくり取得。クラス名は”cassetteRecruit cassetteUseFloat”であるためCSSセレクタで指定。
    recruit_articles=driver.find_elements(by=By.CSS_SELECTOR,value=".cassetteRecruit cassetteUseFloat")
    ...
    '''
    # recruit_articlesからrecruit_articleに１つずつ取り出し。
    
    for recruit_article in recruit_articles:
        # print(name_elm.text)
        # DataFrameに対して辞書形式でデータを追加する
        name_elm = driver.find_element(by=By.CLASS_NAME, value="cassetteRecruit__name")
        # キャッチコピー部分を取得する。クラス名で要素を取得し、copy_elmに格納
        copy_elm = driver.find_element(by=By.CLASS_NAME, value="cassetteRecruit__copy boxAdjust")
        df = df.append(
            {"会社名": name_elm.text,
